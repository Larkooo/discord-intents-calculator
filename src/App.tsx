...
function App() {
    ...
    const initialState = {
        ...
        privilegedIntents: {
            presence:
                localStorage.getItem("presenceIntent") === "true" || false,
            guildMembers:
                localStorage.getItem("guildMembersIntent") === "true" || false,
            messageContent:
                localStorage.getItem("messageContentIntent") === "true" ||
                false,
        },
        intents: 0,
    ...
    const handleChange = (event: React.ChangeEvent<HTMLInputElement>): void => {
        ...
        if (
            event.target.name !== "presence" &&
            event.target.name !== "guildMembers" &&
            event.target.name !== "messageContent"
        ) {
            intentsValue +=
                (event.target.checked ? 1 : -1) <<
                intents[event.target.name][1];
            event.target.checked
                ? (eventsCountValue += intents[event.target.name][0].length)
                : (eventsCountValue -= intents[event.target.name][0].length);
            setState({
                ...state,
            ...
            if (event.target.name === "presence") {
                setState({
                    ...
                    privilegedIntents: {
                        ...
                        guildMembers: state.privilegedIntents.guildMembers,
                        messageContent: state.privilegedIntents.messageContent,
                    },
                });
                ...
                setState({
                    ...state,
                    privilegedIntents: {
                        messageContent: event.target.checked,
                        guildMembers: state.privilegedIntents.guildMembers,
                        presence: state.privilegedIntents.presence,
                    },
                });
                localStorage.setItem(
                ...
                setState({
                    ...state,
                    privilegedIntents: {
                        guildMembers: event.target.checked,
                        presence: state.privilegedIntents.presence,
                        messageContent: state.privilegedIntents.messageContent,
                    },
                });
        ...
        <ThemeProvider theme={theme}>
            ...
            <Container className={classes.align}>
                    ...
                    <Grid item xs={4}>
                        <Paper elevation={5} className={classes.paper}>
                            <Typography variant="h6">
                                What you have access to ({state.eventsCount}{" "}
                                events)
                            </Typography>
                ...
                <Typography variant="caption">
                    Click on this button to copy
                </Typography>

                <p style={{ color: "gray" }}>
                    Copyright © {new Date().getFullYear()} Nasr AA Djeghmoum -
                    Larko#0742 -{" "}
                    <Link
                        href="https://github.com/Larkooo/discord-intents-calculator"
                        underline="always"
                        target="_blank"
                    >
                        Github
